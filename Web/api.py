...
def submit_saisie_hebdo(groupe_id: int, data: SaisieHebdomadaireRequest, cursor=Depends(get_db_cursor)):
    ...
    for saisie in data.enregistrements:
        ...
        final_social = float(saisie.caisse_sociale)

        cursor.execute("""
                UPDATE membres 
                SET solde_epargne = solde_epargne + %s, caisse_sociale = caisse_sociale + %s, status_presence = %s 
                WHERE id = %s
            """, (final_epargne, final_social, saisie.presence, saisie.membre_id))
        ...
        cursor.execute("""
            ...
            """, (saisie.membre_id, groupe_id, data.date_reunion, heure_actuelle, final_epargne, final_social,
                  data.enregistre_par))
